...
const SummaryPage: React.FC = () => {
  ...
  const disabled = !formData.name || !formData.email || !formData.phone || !formData.salary || isSubmitting;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-10 border-gray-200 border-2">
      ...
      {(formData.name === "" || formData.email === "" || formData.phone === "" || formData.salary === "") && <p className="text-red-500">Please fill all the fields</p>}
      <div className="grid grid-cols-2 gap-4">
        <button type="button" className="bg-gray-300 text-gray-800 py-2 px-4 mt-4 rounded" onClick={navigateToPrevious}>
          Previous
        </button>
        <button type="submit" disabled={disabled} className={`bg-green-500 text-white py-2 px-4 mt-4 rounded ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>Submit</button>
      </div>
    </form>
  );
};
